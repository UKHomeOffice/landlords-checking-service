...
echo "$GH_COMMENT_RESPONSE" | jq -e . >/dev/null 2>&1 || { echo "GitHub comment API returned non-JSON response"; exit 1; }
GH_COMMENT_URL="$(echo "$GH_COMMENT_RESPONSE" | jq -r '.html_url // empty')"
test -n "$GH_COMMENT_URL" || { echo "Failed to create GitHub PR comment"; exit 1; }
